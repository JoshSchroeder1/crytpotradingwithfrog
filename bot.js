...
const ALP_KEY    = 'PK5FL25J7OJI6T6UFYOU6PDOTB';
const ALP_SECRET = 'HzdymFuHSadZJyV2n9f1mYkx66LBgBBThYfxPVgrkj69';
const ALP_REST   = 'https://data.alpaca.markets/v1beta3/crypto/us';
const ALP_HDR    = {'APCA-API-KEY-ID': ALP_KEY, 'APCA-API-SECRET-KEY': ALP_SECRET};

const ALP_SYMS = {
  btc:'BTC/USD', eth:'ETH/USD', sol:'SOL/USD', bnb:'BNB/USD',
  xrp:'XRP/USD', ada:'ADA/USD', avax:'AVAX/USD', dot:'DOT/USD',
  link:'LINK/USD', matic:'MATIC/USD', uni:'UNI/USD', atom:'ATOM/USD',
...
async function fetchAlpacaPrices(){
  try{
    const symbols = Object.values(ALP_SYMS).join(',');
    const url = `${ALP_REST}/latest/trades?symbols=${encodeURIComponent(symbols)}`;
    const res = await fetch(url, {headers: ALP_HDR});
    if(!res.ok) throw new Error(`HTTP ${res.status}: ${await res.text()}`);
    const data = await res.json();
    const trades = data.trades || {};
    let updated = 0;
    Object.entries(trades).forEach(([sym, trade])=>{
      const id = Object.keys(ALP_SYMS).find(k=>ALP_SYMS[k]===sym);
      if(!id) return;
      const price = parseFloat(trade.p);
      if(price > 0){
        live[id] = price;
        if(!open24h[id]) open24h[id] = price;
        const c = COINS.find(x=>x.id===id);
        if(c){ c.p=price; c.c=((price-open24h[id])/open24h[id])*100; }
        updated++;
      }
    });
    if(updated > 0) log('info', `[Alpaca] ${updated} prices. BTC=$${live['btc']?.toLocaleString()}`);
    return updated > 0;
  }catch(e){
    log('warn', `[Alpaca] Price fetch failed: ${e.message}`);
    return false;
  }
}

// ── PRICE SIMULATION (fallback only) ─────────────────────────────────────────
// Used only to seed priceHistory for momentum calc between Alpaca fetches
function tickPrices() {
  COINS.forEach(c => {
...
async function scan() {
  ...
  // Fetch real prices from Alpaca first
  await fetchAlpacaPrices();

  // Simulate small intra-scan ticks for momentum tracking between API calls
  tickPrices();
...
async function main() {
  ...
  // Seed price history with warm-up ticks
  console.log('  Warming up price history (25 ticks)…');
  for (let i = 0; i < 25; i++) tickPrices();

  // Fetch real prices from Alpaca before first scan
  console.log('  Fetching live prices from Alpaca…');
  await fetchAlpacaPrices();
